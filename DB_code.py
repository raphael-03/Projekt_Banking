...
def register_user(vorname, nachname, email, alter, bankinstitut, password):
    user_exists = execute_sql("SELECT * FROM kunde WHERE email = %s",
                              (email,), fetch=True)
    if user_exists:
    ...
    if user_email:
        execute_sql("INSERT INTO password (email, password) VALUES (%s, %s)",
...
def konto_anzeigen(email):
    return execute_sql("SELECT name FROM konto_anlegen WHERE email = %s",
                       (email,), fetch=True)

def pruefe_konto(email):
    konto_existiert = execute_sql("SELECT * FROM konto_anlegen WHERE email =%s",
                                  (email,), fetch=True)
    if konto_existiert:
        return True
    return False
...
def finde_kontoid_durch_namen(name, email):
    ergebnis = execute_sql("SELECT kontoid FROM konto_anlegen WHERE name = %s AND email = %s",
...
def letzten_kontoeintraege_zeigen(email, kontoid, anzahl_eintraege):
    if anzahl_eintraege == -1:  # Alle Einträge anzeigen
        eintrag = execute_sql("SELECT Zeitstempel, Betrag, Name_Empfaenger, Verwendungszweck, kategorien.name FROM kontoeintrag LEFT JOIN kategorien ON kontoeintrag.kategorienid = kategorien.kategorienid WHERE kontoeintrag.email = %s AND kontoeintrag.kontoid = %s ORDER BY zeitstempel",
                              (email, kontoid), fetch=True)

    else:
        eintrag = execute_sql("SELECT Zeitstempel, Betrag, Name_Empfaenger, Verwendungszweck, kategorien.name FROM kontoeintrag LEFT JOIN kategorien ON kontoeintrag.kategorienid = kategorien.kategorienid WHERE kontoeintrag.email = %s AND kontoeintrag.kontoid = %s ORDER BY zeitstempel LIMIT %s",
                              (email, kontoid, anzahl_eintraege), fetch=True)
    return eintrag
def finde_kontoid_name_email(email, name):
    kontid = execute_sql(" SELECT kontoid FROM konto_anlegen WHERE email=%s AND name =%s",
                         (email, name), fetch=True)
    return kontid
def letzten_kontoeintraege_zeigen_5(email):
    eintrag =execute_sql("SELECT Zeitstempel, Betrag, Name_Empfaenger, Verwendungszweck, kategorien.name FROM kontoeintrag  LEFT JOIN kategorien  ON kontoeintrag.kategorienid = kategorien.kategorienid WHERE kontoeintrag.email = %s ORDER BY zeitstempel DESC LIMIT 5",
                         (email,), fetch=True)
    return eintrag

def kategorien_erstellen(email, name):
    pruefe = execute_sql("SELECT name FROM kategorien WHERE name=%s",
                         (name,), fetch=True)
    if pruefe:
        return 'Kategorie existiert bereits'
    else:
        kategorien = execute_sql("INSERT INTO kategorien(email, name) VALUES (%s, %s)",
                                 (email, name,))
        return kategorien

def kategorien_waehlen(email):
    return execute_sql("SELECT name,kategorienid FROM kategorien WHERE email=%s",
                       (email,), fetch=True)

def schlagwort_einfuegen(kategorienid, wort):
    return execute_sql("INSERT INTO schlagwoerter(kategorienid, wort) VALUES (%s, %s)",
                       (kategorienid, wort))

def kategorien_erstellen_2(email, name, schlagwoerter):
    execute_sql("INSERT INTO kategorien(email, name) VALUES (%s, %s);",
                (email, name))
    ergebnis = execute_sql("SELECT * FROM kategorien WHERE email=%s AND name=%s",
                           (email, name), fetch=True)
    if ergebnis:
        ...
        for wort in schlagwoerter:
            ...
            execute_sql("INSERT INTO schlagwoerter(kategorienid, wort) VALUES (%s, %s)",
                        (kategorienid, wort))
...
def kategorie_zu_kontoeintraegen_zuordnen(kategorienid, schlagwoerter):
    for wort in schlagwoerter:
        ...
        eintraege = execute_sql("SELECT kontoeintragid FROM Kontoeintrag WHERE Verwendungszweck LIKE %s",
                                ('%' + wort + '%',), fetch=True)
        for eintrag in eintraege:
            # Aktualisieren den Kontoeintrag mit der kategorienid
            execute_sql("UPDATE Kontoeintrag SET kategorienid=%s WHERE kontoeintragid=%s",
                        (kategorienid, eintrag[0]))
...
def excel_export(email,kontoid):
    sql_excel =execute_sql("SELECT Zeitstempel, Betrag, Name_Empfaenger, Verwendungszweck, kategorien.name FROM kontoeintrag LEFT JOIN kategorien ON kontoeintrag.kategorienid = kategorien.kategorienid WHERE kontoeintrag.email = %s AND kontoeintrag.kontoid = %s ORDER BY zeitstempel",
                           (email, kontoid),fetch=True)
...
def finde_konto_name(email, kontoid):
    konto_name = execute_sql("SELECT name FROM konto_anlegen WHERE email = %s AND kontoid = %s",
                             (email,kontoid,), fetch=True)
